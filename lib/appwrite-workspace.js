import { Client, Databases, ID, Query, Permission, Role, Messaging, Account } from "appwrite";
...
export const toggleWorkspacePublic = async (workspaceId, isPublic, userEmail = null) => {
  try {
    ...
    if (isPublic && userEmail) {
      const shareUrl = `${process.env.APPWRITE_ENDPOINT}/.../workspace/${shareToken}`;
...
const sendWorkspaceNotification = async (
  ...
  try {
    ...
    await messaging.createEmail(
      `workspace-shared-${Date.now()}`, // messageId
      userEmail,
      `Your workspace "${workspaceTitle}" is now public!`,
      `Your Mermaid diagram workspace "${workspaceTitle}" has been made public and can be shared with others.\n\nShare link: ${shareUrl}\n\nVisit your workspace to view and edit it.`
    );
...
// Export basic Appwrite instances for backward compatibility
